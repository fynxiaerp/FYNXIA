...
**Last activity:** 2026-07-19 - Completed quick task 260719-dkz: corrigir UUID cru no Select de dentista + validação invisível em ProfessionalForm
...
Ficou de fora (não corrigido, escopo futuro): tabela de listagem de Profissionais com células de nome vazias e "Unidade"/"Login vinculado" mostrando UUID (mesmo padrão de bug); `users.full_name` vazio para os 2 usuários dentista de teste (impede o nome de aparecer no Select de "Nova Consulta" mesmo após o fix).

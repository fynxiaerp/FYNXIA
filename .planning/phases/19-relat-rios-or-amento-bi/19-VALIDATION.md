...
|----------|-------------|------------|-------------------|
| DRE drill-down UI navigation (click line → filtered transactions list) | REP-01 | Visual/interaction flow, not a pure function | Load `/clinica/relatorios`, click a DRE line, confirm filtered `financial_transactions` list opens with matching account_id+período+unidade |
